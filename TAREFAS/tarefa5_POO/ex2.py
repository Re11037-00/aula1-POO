...
class BoletoUI:
    ...
    @classmethod #4
    def excluir(cls):
        cod = int(input("informe o código do boleto a ser excluído: "))
        for x in cls.__boletos: 
            if x.get_cod_barras() == cod:
    ...
    def a_vencer(cls):
        for x in cls.__boletos: 
            if x.get_situacao_pagamento() == Pagamento.EM_ABERTO or x.get_situacao_pagamento() == Pagamento.PAGO_PARCIAL and \
    ...
    def pagar(cls):
        for x in cls.__boletos: print(x)
        cod = int(input("informe o código do boleto a ser pago: "))
        for x in cls.__boletos: 
            if x.get_situacao_pagamento() == Pagamento.PAGO: raise ValueError("Esse Boleto já foi pago")
